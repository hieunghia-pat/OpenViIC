...
from models.camo_tuning_transformer import Transformer, TransformerEncoder, TransformerDecoderLayer, ScaledDotProductAttention
import torch
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='CAMO Transformer')
    ...
    print('CAMO Transformer Training')
    ...
    if not os.path.isfile(os.path.join(args.saved_folder, 'vocab_%s.pkl' % args.exp_name)):
        print("Building vocabulary")
        text_field.build_vocab(train_dataset, val_dataset, min_freq=5)
        pickle.dump(text_field.vocab, open(os.path.join(args.saved_folder, 'vocab_%s.pkl' % args.exp_name), 'wb'))
    else:
    ...
    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = os.path.join(args.saved_folder, '%s_last.pth' % args.exp_name)
        else:
            fname = os.path.join(args.saved_folder, '%s_best.pth' % args.exp_name)
    ...
    while True:
        ...
        torch.save({
            ...
            'use_rl': use_rl,
        }, os.path.join(args.saved_folder, '%s_last.pth' % args.exp_name))
