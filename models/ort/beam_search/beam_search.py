...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, boxes, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                outputs = self.iter(t, visual, boxes, outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, boxes, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, visual, boxes, None, mode='feedback', **kwargs)
