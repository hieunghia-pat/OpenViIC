...
class BertAdaptiveLanguageModel(Module):
    def __init__(self, padding_idx=0, pretrained_name="bert-base-uncased", bert_hidden_size=768, vocab_size=10201, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, max_len=54, dropout=.1):
        super(BertAdaptiveLanguageModel, self).__init__()
        self.padding_idx = padding_idx
        self.d_model = d_model

        self.language_model = BertModel.from_pretrained(pretrained_name, return_dict=True)
...
class AlbertAdaptiveLanguageModel(Module):
    def __init__(self, padding_idx=0, pretrained_name="vinai/phobert-base", bert_hidden_size=768, vocab_size=10201, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, max_len=54, dropout=.1):
        super(AlbertAdaptiveLanguageModel, self).__init__()
        self.padding_idx = padding_idx
        self.d_model = d_model

        self.language_model = AlbertModel.from_pretrained(pretrained_name, return_dict=True)
        self.language_model.config.vocab_size = vocab_size
        self.proj_to_caption_model = nn.Linear(bert_hidden_size, d_model)
