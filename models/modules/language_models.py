...
class PhoBERTModel(Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None):
        ...
        pretrained_language_idx_mapping = json.load(open(os.path.join(config.checkpoint_path, config.model_name, "pretrained_language_idx_mapping.json")))
        pretrained_padding_idx = self.token_encoder.convert_tokens_to_ids(self.token_encoder.pad_token)
        for input_id in input_ids:
            pretrained_input_id = [pretrained_language_idx_mapping[str(ori_id.item())] for ori_id in input_id]
        ...
        mask_queries = generate_padding_mask(pretrained_input_ids, pretrained_padding_idx).to(pretrained_input_ids.device)  # (b_s, seq_len)
        mask_self_attention = generate_sequential_mask(seq_len).to(pretrained_input_ids.device)
        mask_self_attention = mask_self_attention.unsqueeze(0).unsqueeze(0)  # (1, 1, seq_len, seq_len)
        mask_self_attention = torch.logical_or(mask_self_attention, mask_queries.unsqueeze(1).unsqueeze(1))
